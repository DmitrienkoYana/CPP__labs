...
bool GameGrid::checkSequence(int row, int col) {
    ...
    for (int i = row - 1; i <= row + 1; i++) {
        if(i >= 0 && i < GRID_SIZE){
            if(this->grid[i][col]->type == -1 * this->grid[row][col]->type) 
                return true;
    ...
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            if (this->grid[i][j]->type < 0){
                ...
                if (randBonus == 0)  {
                    addBonuses(i, j, new BombBonus());
                }else if (randBonus == 1)  {
                    addBonuses(i, j, new PaintBonus(this->grid[i][j]->type));
                }
...
void GameGrid::addBonuses(int row, int col, Bonus* bonus) {
    bool add = true;
    while (add) {
        int rowAdd = row + rand() % 5 - 2;
        int colAdd = col + rand() % 5 - 2;

        if (rowAdd >= 0 && rowAdd < GRID_SIZE && colAdd >= 0 && colAdd < GRID_SIZE) {
            if (grid[rowAdd][colAdd]->type <= 0) {
                delete grid[rowAdd][colAdd];
                grid[rowAdd][colAdd] = bonus;
                add = false;
            }
        }
    }
    ...
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            ...
            if (bonus != nullptr) {
                ...
                grid[i][j] = new Square( COLORS[0][0],  COLORS[0][1], COLORS[0][2], -1);
                this->draw();
            }
        }
    }
